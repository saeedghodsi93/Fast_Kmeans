...
float calculateDistance(int p1[dim], float p2[dim])
  {
    ...
    int i;
    for (i = 0; i < dim; i++)
		distance += ((float)p1[i] - p2[i])*((float)p1[i] - p2[i]);                        // change
...
void calculateClustersCentroids(int x[n][dim], int cluster_assignment_index[n], float new_cluster_centroid[k][dim])
  {
    int i,j;
	...
	for (i = 0; i < k; i++)
    {
        if (cluster_member_count[i] != 0)
			for (j = 0; j < dim; j++)
				new_cluster_centroid[i][j] /= (float)cluster_member_count[i];         //change
    }
  }
