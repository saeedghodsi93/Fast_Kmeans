...
// Define the states of state machine
   ...
   localparam Read_n_k_dim = 5'd1;
   localparam Read_centers  = 5'd2;
   localparam Read_center_point = 5'd3;
   localparam Read_center_point_mem_write_wait  = 5'd4;
   localparam Read_center_point_mem_write = 5'd5;
   localparam Read_first  = 5'd6;
   localparam Read_first_point  = 5'd7;
   localparam Read_first_point_mem_write_wait  = 5'd8;
   localparam Read_first_point_mem_write  = 5'd9;
   localparam Update  = 5'd10;
   localparam Update_point  = 5'd11;
   localparam Read_a_read_wait  = 5'd12;
   localparam read_a_get_input = 5'd13;
   localparam read_a_write_wait  = 5'd14;
   localparam read_a_write_input  = 5'd15;
   localparam Start_process  = 5'd16;
   localparam Read_next  = 5'd17;
   localparam Read_next_point  = 5'd18;
   localparam Read_next_point_mem_write_wait  = 5'd19;
   localparam Read_next_point_mem_write  = 5'd20;
   localparam Process_wait  = 5'd21;
   localparam Write_output  = 5'd22;
	...
	wire	[8:0] mem_input_address_1, mem_input_address_2, mem_a_address_1;
	...
	reg	s_mem_input_write_enable_1, s_mem_a_write_enable_1, s_mem_b_write_enable_1;
	reg	[31:0] s_mem_input_input_data_1, s_mem_input_input_data_2, s_mem_a_input_data_1, s_mem_b_input_data_1;
	reg	[8:0] s_mem_input_address_1, s_mem_input_address_2, s_mem_a_address_1;
	reg	[13:0] s_mem_b_address_1;
	
	reg s_min_ack, s_min_start;
	reg [0:31] n, k, dim;
   reg [0:2] state;
   reg [0:31] out;

   // Counters to store the number inputs read & outputs written
   reg [0:10 - 1] nr_of_reads;
   reg [0:10 - 1] nr_of_writes;
   reg [0:10 - 1] nr_of_centers;
   ...
   begin  // process The_SW_accelerator
          ...
          Read_n_k_dim: 
            ...
            begin
				...
				begin
					...
					nr_of_reads <= 10'd0;
					s_mem_b_address_1 <= nr_of_centers << 9;
				end
		    end
			
			 Read_center_point: 
				begin
				s_mem_b_address_1 <= s_mem_b_address_1[13:0];
            if (FSL_S_Exists == 1) 
            begin
              if (nr_of_reads >= dim)
              begin
						state <= Read_centers;
						nr_of_centers <= nr_of_centers + 10'd1;
              end
              else
				  begin
			 ...
			 Read_center_point_mem_write:
				...
				state <= Read_center_point;
				nr_of_reads <= nr_of_reads + 10'd1;
				s_mem_b_address_1 <= s_mem_b_address_1 + 10'd1;
            ...
            begin
			 ...
			 Read_first_point_mem_write:
				...
				state <= Read_first_point;
				nr_of_reads <= nr_of_reads + 10'd1;
				s_mem_input_address_1 <= s_mem_input_address_1 + 10'd1;
			 ...
			 Read_a_read_wait:
			 begin
				state <= read_a_get_input;
			 end
			
			 read_a_get_input:
		 	 begin
				s_mem_a_input_data_1 <= mem_input_output_data_2;
				s_mem_a_write_enable_1 <= 1'b1;
				state <= read_a_write_wait;
			 end
			
			 read_a_write_wait:
			 begin
				state <= read_a_write_input;
			 end
	...
	assign FSL_S_Read  = ((state == Read_center_point) || (state == Read_first_point) || (state == Read_next_point) || (state == Read_n_k_dim)) ? FSL_S_Exists : 0;
   assign FSL_M_Write = (state == Write_output) ? ~FSL_M_Full : 0;

	assign min_clock = FSL_Clk;
	assign min_reset = FSL_Rst;
	assign min_k = k[0:5];
	assign min_dim = dim[0:9];
	assign min_ack = s_min_ack;
	assign min_start = s_min_start;
	assign mem_a_write_enable_1 = s_mem_a_write_enable_1;
	assign mem_b_write_enable_1 = s_mem_b_write_enable_1;
	assign mem_input_input_data_1 = s_mem_input_input_data_1;
	assign mem_input_input_data_2 = s_mem_input_input_data_2;
